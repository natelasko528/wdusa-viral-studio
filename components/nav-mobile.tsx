...
const TABS = [
  { href: "/studio", label: "Studio", icon: ICON_STUDIO },
  { href: "/kb", label: "Knowledge Base", icon: ICON_KB },
  { href: "/templates", label: "Templates", icon: ICON_TEMPLATES },
  { href: "/chat", label: "Chat", icon: ICON_CHAT },
...
export function NavMobile() {
  ...
  return (
    <nav
      className="fixed bottom-0 left-0 right-0 z-30 flex border-t border-[var(--border)] bg-[var(--surface-raised)]/95 backdrop-blur-md md:hidden"
    ...
    >
      {TABS.map((item) => {
        ...
        return (
          ...
          >
            {item.icon}
            <span className="truncate max-w-[56px]">{item.label === "Knowledge Base" ? "KB" : item.label}</span>
          </Link>
        );
      })}
    </nav>
